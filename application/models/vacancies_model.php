    ...
    **************************************/
    class Vacancies_model extends CI_Model {
        ...
        function getJSONList(){
            ...
            $sql = 'SELECT * FROM `cmsvacancies` ORDER BY position';
            ...
            if($query->num_rows() > 0){
                foreach($query->result() as $row){
                    ...
                    $edit_url = site_url('ControlPanel/vacancies/details/'.$row->ID);
                    $delete_url = site_url('ControlPanel/vacancies/delete/'.$row->ID);
        ...
        function get_search_list($page){
			...
			(integer) $page;
			if($page == 0){
				$sql = 'SELECT * FROM `cmsvacancies` WHERE `approved` = 1 ORDER BY `position` ASC, `date` DESC LIMIT 10';
			}else{
				$sql = 'SELECT * FROM `cmsvacancies` WHERE `approved` = 1 ORDER BY `position` ASC, `date` DESC LIMIT '.$page.', 10';
			}
			
			$query = $this->db->query($sql);
			$htm = '<ul class="vacancies">';
			...
			if($query->num_rows > 0){
				foreach($query->result() as $row){                    
					...
					$href = 'href="'.base_url().'careers/vacancies_details/'.$id.'/"';
		...
		function get_count(){                          
            $sql = 'SELECT COUNT(`ID`) AS `cnt` FROM `cmsvacancies` WHERE `approved` = 1 ORDER BY `position` ASC, `date` DESC';               
        ...
        function save_list_order($items_array){
            ...
            foreach($items_array as $key => $value){
                    $item_id = $value['id'];
                    $sql = 'UPDATE cmsvacancies SET position = '.$i.' WHERE ID = '.$item_id.' LIMIT 1';
                    $query = $this->db->query($sql);
                    $i++;
            }
        }
        ...
        function delete($id){
            (integer) $id;
            $sql = 'DELETE FROM cmsvacancies WHERE ID = '.$id.' LIMIT 1';
            $query = $this->db->query($sql);
            return true;
        }
        ...
        function update($id, $approved, $title_en, $title_az, $tagline_en, $text_en, $tagline_az, $text_az){
            ...
            $sql = 'UPDATE `cmsvacancies` SET `approved` = "'.$approved.'", `date` = NOW(), `title_en` = "'.$title_en.'", `title_az` = "'.$title_az.'", `tagline_en` = "'.$tagline_en.'", `text_en` = "'.$text_en.'", `tagline_az` = "'.$tagline_az.'", `text_az` = "'.$text_az.'" WHERE ID = '.$id.' LIMIT 1';
            $query = $this->db->query($sql);
            return true;
        }
        ...
        function getDetails($id){
            (integer) $id;
            $sql = 'SELECT * FROM `cmsvacancies` WHERE `ID`='.$id.' LIMIT 1';
            $query = $this->db->query($sql);
        ...
        function add($approved, $title_en, $title_az, $tagline_en, $text_en, $tagline_az, $text_az, $section, $sector){            
            ...
            $sql = 'INSERT INTO cmsvacancies(`approved`, `date`, `title_en`, `title_az`, `tagline_en`, `text_en`, `tagline_az`, `text_az`, `section`, `sector`) VALUES
        ...
        function get_form_values($cur_value=0){
        	$sql = 'SELECT * FROM `cmsvacancies` WHERE `approved` = 1';
        	$query = $this->db->query($sql);
			...
			foreach ($values as $key => $value){
				if ($value['ID'] == $cur_value) {
					$list .= '<option selected="selected" value="'.$value['ID'].'">'.$value['title_en'].'</option>';
				}else{
					$list .= '<option value="'.$value['ID'].'">'.$value['title_en'].'</option>';
				}
			}
