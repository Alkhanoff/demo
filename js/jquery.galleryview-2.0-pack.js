...
(function ($) {
    $.fn.galleryView = function (g) {
        ...
        function showItem(i) {
            ...
            if (h.show_filmstrip) {
                if (m == 'strip') {
                    ...
                    var a, diststr, old_i;
                    if (F == 'horizontal') {
                        a = getPos(O[i]).left - (getPos(R[0]).left + (u / 2) - (A / 2));
                        diststr = (a >= 0 ? '-=' : '+=') + Math.abs(a) + 'px';
                        N.animate({
                            'left': diststr
                            ...
                            if (old_i != i) {
                                O.eq(old_i).removeClass('current').find('img').css({
                                    'opacity': h.frame_opacity
                                });
                        ...
                        a = getPos(O[i]).top - (getPos(R[0]).top + (t) - (B / 2));
                        diststr = (a >= 0 ? '-=' : '+=') + Math.abs(a) + 'px';
                        N.animate({
                            'top': diststr
                        }, h.transition_speed, h.easing, function () {
        ...
        function buildPanels() {
            ...
            if (!h.show_filmstrip) {
                $('<img />').addClass('nav-next').attr('src', n + h.nav_theme + '/next.gif').appendTo(M).css({
                    ...
                    'right': 10 + getInt(Q.css('borderRightWidth')) + 'px',
                    'display': 'none'
                }).click(showNextItem);
                $('<img />').addClass('nav-prev').attr('src', n + h.nav_theme + '/prev.gif').appendTo(M).css({
                    ...
                    'cursor': 'pointer',
                    'top': ((h.panel_height - 22) / 2) + D + 'px',
                    'left': 10 + getInt(Q.css('borderLeftWidth')) + 'px',
                    'display': 'none'
                }).click(showPrevItem);
                $('<img />').addClass('nav-next-overlay').attr('src', n + h.nav_theme + '/panel-nav-next.gif').appendTo(M).css({
                    ...
                    'right': getInt(Q.css('borderRightWidth')) + 'px',
                    'display': 'none',
                    'cursor': 'pointer',
                    'opacity': 0.75
                }).click(showNextItem);
                $('<img />').addClass('nav-prev-overlay').attr('src', n + h.nav_theme + '/panel-nav-prev.gif').appendTo(M).css({
                    ...
                    'zIndex': '1099',
                    'top': ((h.panel_height - 22) / 2) + D - 10 + 'px',
                    'left': getInt(Q.css('borderLeftWidth')) + 'px',
                    'display': 'none',
                    'cursor': 'pointer',
                    'opacity': 0.75
                }).click(showPrevItem)
            ...
            if (K) {
                $('img', Q).each(function (i) {
                    $(this).css({
                        ...
                        'width': H[i % l] * J[i % l],
                        'position': 'relative',
                        'top': (h.panel_height - extraHeight(Q) - (H[i % l] * I[i % l])) / 2 + 'px',
                        'left': (h.panel_width - extraWidth(Q) - (H[i % l] * J[i % l])) / 2 + 'px'
                    })
                })
            }
        };
        ...
        function buildFilmstrip() {
            ...
            if (F == 'horizontal') {
                c.css({
                    'top': (h.filmstrip_position == 'top' ? Math.max(D, E) : h.panel_height + E + D) + ((B - 22) / 2) - 280 + 'px',
                ...
                c.css({
                    'left': (h.filmstrip_position == 'left' ? Math.max(D, E) : h.panel_width + E + D) + ((A - 22) / 2) + 18 + 'px',
                    'top': wrapper_height + (D * 2) + 'px'
                });
                d.css({
                    'left': (h.filmstrip_position == 'left' ? Math.max(D, E) : h.panel_width + E + D) + ((A - 22) / 2) - 18 + 'px',
                    'top': wrapper_height + (D * 2) + 'px'
        ...
        function buildGallery() {
            ...
            a.each(function (i) {
                ...
                if (h.panel_scale == 'nocrop') {
                    H[i] = Math.min((h.panel_height - extraHeight(Q)) / I[i], (h.panel_width - extraWidth(Q)) / J[i])
                } else {
                    H[i] = Math.max((h.panel_height - extraHeight(Q)) / I[i], (h.panel_width - extraWidth(Q)) / J[i])
                }
            });
            M.css({
                'position': 'relative',
        ...
        return this.each(function () {
            ...
            $('script').each(function (i) {
                ...
                if (s.attr('src') && s.attr('src').match(/jquery\.galleryview/)) {
                    ...
                    n = s.attr('src').split('jquery.galleryview')[0] + 'themes/'
                }
            });
            N = $('.filmstrip', M);
            O = $('li', N);
            ...
            if (F == 'horizontal') {
                ...
                strip_size = h.show_panels ? Math.floor((h.panel_height - (D + 22)) / (((B * l) + (h.frame_gap * (l - 1))) / l)) : Math.min(l, h.filmstrip_size)
            }
            if (strip_size >= l) {
                m = 'pointer';
                strip_size = l
