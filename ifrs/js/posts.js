...
function loadPost()
{
	...
	contentF.location = courseWin.baseURL+"/units/"+pageSplit[0]+"/"+pageSplit[1]+"/content/post_"+currentPage;
}

function checkHistory()
{
	var found = false;
	if (courseObj[standard].postHistory != null)
	{
		for (var i=0; i<courseObj[standard].postHistory.length; i++)
		{
			if (currentPage == courseObj[standard].postHistory[i][0])
			{
				inHistory = true;
				break;
			}
	...
	if (!inHistory)
		courseWin.insertPostHistory(contentF.subject);
}
...
function showHelp() {
	...
	if (c.popupShowing == "help") {
		...
		if (c.document.getElementById("popupContent")) {
			...
			showPopupLayer();
			strHelp = popupTitle('help', 'left') + popupDiv(window.opener.helpText("comm_post"), 340, 222, true);
