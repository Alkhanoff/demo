...
function loadPostit()
{
	...
	contentF.location = courseWin.baseURL+"/units/"+pageSplit[0]+"/"+pageSplit[1]+"/content/postit_"+currentPage;
}

function checkHistory()
{
	if (courseObj[standard].postitHistory != null)
	{
		for (var i=0; i<courseObj[standard].postitHistory.length; i++)
		{
			if (currentPage == courseObj[standard].postitHistory[i][0])
			{
				inHistory = true;
				break;
			}
	...
	if (!inHistory)
		courseWin.insertPostitHistory(contentF.subject);
}

function printPage() {
	top.printWhichFrame = top.contentF;
	top.printType = "postit";
...
function showHelp() {
	...
	if (c.popupShowing == "help") {
		...
		if (c.document.getElementById("popupContent")) {
			...
			showPopupLayer();
			strHelp = popupTitle('help', 'left') + popupDiv(window.opener.helpText("comm_postit"), 340, 222, true);
