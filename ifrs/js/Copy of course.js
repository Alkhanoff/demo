...
function continueInit()
{
	...
	if (web)
		...
		document.frames['mainArea'].location = baseURL + "/generichtm/offline/offline.htm";
}

function continueNormal()
{
	if ((courseID == "ias3239") || (courseID == "ias32392") || (courseID == "ias32393"))
		document.frames['mainArea'].location = baseURL+"/"+standard+"/intro.htm";
	else
		loadMenu();
}
...
function loadActionPlanner()
{
	...
	{
		if (appVersion[i].toLowerCase().indexOf("windows nt",0) != -1)
		{
			appVersionNum = appVersion[i].split(" ")[3] * 1;
			if (appVersionNum >= 6)
			{
				isVista = true;
			}
...
/*
	...
	if (plannerWin != null)
	{
		if (!plannerWin.closed)
			plannerWin.close();

		plannerWin = null;
	}

	plannerWin = window.open(top.baseURL+"/generichtm/actionplanner.htm","plannerInstWin","width=600,height=450,left="+x+",top="+y+",toolbar=no,menubar=no,status=no,scrollbars=no,resizable=no,fullscreen=no");
*/

	if (isVista)
		alert("The Action planner is currently incompatible with Vista. If you are accessing the modules with this software, you will need to open MS Word and enter any notes manually.");
	else
