...
function loadMemo()
{
	...
	contentF.location = courseWin.baseURL+"/units/"+pageSplit[0]+"/"+pageSplit[1]+"/content/memo_"+currentPage;
}

function checkHistory()
{
	if (courseObj[standard].memoHistory != null)
	{
		for (var i=0; i<courseObj[standard].memoHistory.length; i++)
		{
			if (currentPage == courseObj[standard].memoHistory[i][0])
			{
				inHistory = true;
				break;
			}
	...
	if (!inHistory)
		courseWin.insertMemoHistory(contentF.subject);
}
...
function showHelp() {
	...
	if (c.popupShowing == "help") {
		...
		if (c.document.getElementById("popupContent")) {
			...
			showPopupLayer();
			strHelp = popupTitle('help', 'left') + popupDiv(window.opener.helpText("comm_memo"), 340, 222, true);
