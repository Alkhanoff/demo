...
function loadInterruption()
{
	...
	contentF.location = courseWin.baseURL+"/units/"+pageSplit[0]+"/"+pageSplit[1]+"/content/interruption_"+currentPage;
}

function checkHistory()
{
	var found = false;
	if (courseObj[standard].interruptionHistory != null)
	{
		for (var i=0; i<courseObj[standard].interruptionHistory.length; i++)
		{
			if (currentPage == courseObj[standard].interruptionHistory[i][0])
			{
				inHistory = true;
				break;
			}
	...
	if (!inHistory)
		courseWin.insertInterruptionHistory(contentF.subject);
}

function printPage() {
	top.printWhichFrame = top.contentF;
	top.printType = "interruption";
...
function showHelp() {
	...
	if (c.popupShowing == "help") {
		...
		if (c.document.getElementById("popupContent")) {
			...
			showPopupLayer();
			strHelp = popupTitle('help', 'left') + popupDiv(window.opener.helpText("comm_interruption"), 340, 222, true);
